...
// Health probe for Terraform readiness
app.get('/health', (req, res) => res.status(200).json({ ok: true }));

// Using AWS S3 SDK compatible with IBM COS
function makeS3(region, cosEndpoint) {
  ...
  // SDK uses env credentials injected by platform (if available) or instance profiles in CE
  const s3 = new AWS.S3({
    endpoint,
...
app.listen(PORT, () => {
  console.log(`Vibe broker listening on ${PORT}`);
});
